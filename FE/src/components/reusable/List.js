...
function List() {
  ...
  const [memberIndex, setMemberIndex] = useState(Number(localStorage.getItem("memberIndex")));
  ...
  const onDetail = (boardIndex, memberIndex) => {
    ...
    return <div>{isStarDetailOpen.length !== 0 && <StarDetail startIndex={boardIndex} userIndex={memberIndex} />}</div>;
  };
          ...
          <Table className="Star-List">
            <TableHeader>
              <TableRow className="font-['Pre-Bold'] bg-white text-m ">
                <TableHead className="text-center w-3/12">일기 등록일</TableHead>
                <TableHead className="text-center w-2/12">지정일</TableHead>
            ...
            <TableBody>
              {filterData.map((it) => (
                <>
                  <TableRow className="font-['Pre-Light'] text-center" key={it.boardIndex}>
                    <TableCell>{it.boardRegTime}</TableCell>
                    <TableCell>{it.boardInputDate}</TableCell>
                    <TableCell onClick={() => onDetail(it.boardIndex, it.memberIndex)}>{it.boardContent}</TableCell>
                    <TableCell>
                      <button onClick={() => deleteStar(it.boardIndex, it.memberIndex)} className="bg-modal-bg w-6/12">
                        X
                      </button>
                    </TableCell>
                  </TableRow>
