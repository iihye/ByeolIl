...
function CloseButton(props) {
    ...
    return (
        <div
            className="text-white-sub hover:cursor-pointer text-3xl"
            onClick={handleClose}
        >
            <CgCloseR />
        </div>
...
function DeleteButton(props) {
    ...
    return (
        <div
            className="text-white-sub hover:cursor-pointer"
...
function ReportButton(props) {
    ...
    return (
        <div
            className="text-white-sub text-3xl hover:cursor-pointer"
...
function LikeButtons(props) {
    ...
    return (
        <>
            {!isLike ? (
                <div
                    className="hover:cursor-pointer text-white-sub "
                    onClick={handleLike}
                >
                    <FaRegHeart />
                </div>
        ...
        <div style={{ display: 'flex' }}>
            {props.data &&
                props.data.boardMedia.map((it, index) => (
                    <img src={it} style={{ width: '50px' }}></img>
                ))}
        </div>
    );
}
...
function RadioContent() {
    ...
    const fetchData = async () => {
        await axios
            .get(
                `${process.env.REACT_APP_API_URL}/radio/${localStorage.getItem(
                    'memberIndex'
                )}`,
                {
                    headers: {
