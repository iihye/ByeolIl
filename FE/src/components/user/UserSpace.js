  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      await axios
        ...
        .then((response) => {
          isAddedStar.clear();
          response.data.BoardListResponseDtoList.forEach((star) => isAddedStar.set(star.boardLocation, star.boardIndex));
...
function StarRegistArea() {
  ...
  return (
    <div>
      ...
      {isStarModifyOpen !== -1 && <StarRegist type={"modify"} preBoard={isStarModifyOpen} />}
    </div>
  );
}
...
function StarDetailArea() {
  ...
  return <div>{isStarDetailOpen.length !== 0 && <StarDetail starIndex={isStarDetailOpen[0]} userIndex={isStarDetailOpen[1]} />}</div>;
}
...
function UserSpace() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      // 내 팔로잉 목록 불러오기
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/follow/following/${loginIndex}`, {
          headers: {
            token: loginToken,
          },
        });
        const isFollow = response.data.result.some((it) => it.memberName === userName);

        isFollow ? setFollowState("언팔로우") : setFollowState("팔로우");
      } catch (error) {
        console.log(error);
      }
    };
