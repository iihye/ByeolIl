...
function Alarm() {
    ...
    const onClose = (index) => {
        ...
        axios
            ...
            .then(
                setAlarmData((currentAlarmData) => currentAlarmData.filter((it) => it.alarmIndex !== index).reverse())
            )
    ...
    useEffect(() => {
        const fetchData = async () => {
            await axios
                .get(`${process.env.REACT_APP_API_URL}/alarm/list/${memberIndex}`)
                .then((response) => {
    ...
    return (
        <div className="outside w-full h-full absolute top-0 left-0 flex justify-center items-center z-10 bg-modal-outside">
            <Card className="Alarm bg-modal-bg text-white-sub px-6 py-6 rounded-component" style={{ width: "480px" }}>
                    ...
                    <ScrollArea className="h-52 font-['Pre-Light'] text-m py-1 px-1.5 pr-5">
                        {alarmData.map((it) => {
                            switch (it.alarmType) {
                                case "FOLLOW":
                                    return (
                                        <div key={it.alarmIndex} className="flex justify-between text-m py-1">
                                            <div className="flex">
                                                <FaUserPlus size="24" className="pr-2 text-btn-bg-hover" />
                                                {it.fromMemberNickName}님이 나를 팔로우했습니다
                                            </div>
                                            <CloseButton onClose={onClose} alarmIndex={it.alarmIndex} />
                                        </div>
                                    );
                                ...
                                case "CMT":
                                    return (
                                        <div key={it.alarmIndex} className="flex justify-between text-m py-1">
                                            <div
                                                className="flex"
                                                onClick={() => setIsStarDetailOpen([it.boardIndex, -2])}
                                            >
                                            ...
                                            {detailModal && boardState === it.boardIndex && (
                                                <div className="fixed top-0 bottom-0 right-0 left-0">
                                                    {<StarDetail starIndex={it.boardIndex} />}
                                                </div>
                                            )}
                                        </div>
                                    );
                                    ...
                                    return (
                                        <div key={it.alarmIndex} className="flex justify-between text-m py-1">
                                            ...
                                            >
                                                ...
                                                {it.fromMemberNickName}님이 내 댓글에 답댓글을 남겼습니다
                                            </div>
                                            <CloseButton onClose={onClose} alarmIndex={it.alarmIndex} />
                                        </div>
                                    );
