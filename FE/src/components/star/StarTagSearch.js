      ...
      <Card className=" w-cardContainer card-contain-style py-3">
        <div className="searchArea flex justify-between items-center search-input mx-auto my-3">
          <div className="flex px-2">
            {replaceTag && (
              <span className="text-black-sub flex items-center p-2" onClick={handleList}>
                #{replaceTag}
                <IoCloseSharp />
              </span>
            )}
            <input name="tagSearch" value={tag} onChange={handleTag} onKeyDown={(e) => activeSearch(e)} className=" text-black-sub my-3" />
          </div>

          <FaSearch size="20" className="text-black-sub mx-3" onClick={() => activeButton()} />
        </div>
            ...
            {tagSearchData ? (
              tagSearchData.length > 0 ? (
                tagSearchData.map((it) => (
                  <Card key={it.boardIndex} className="card-style h-80 w-64">
