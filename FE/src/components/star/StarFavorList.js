...
import { filterState, isFavorListOpenState, isStarDetailOpenState } from "components/atom";
...
function StarFavorList() {
    ...
    const [starDetailState, setStarDetailState] = useRecoilState(isStarDetailOpenState);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/board/like/${memberIndex}`, {
                    headers: {
                        token: token,
                    },
                });
                resetList();
                setListData(response.data.filter((item) => item.boardAccess === "OPEN"));
            } catch (error) {
                console.log(error);
            }
        };
                                ...
                                filterData.length > 0 ? (
                                    filterData.map((it) => (
                                        <Card
                                            ...
                                            onClick={() => setStarDetailState([it.boardIndex, it.memberIndex])}
                                        >
                                            <div className="cards w-4/5 mx-auto font-['Pre-Light']">
                                                ...
                                                <div className="cardContent py-2">{it.boardContent}</div>
                                                <div className="absolute bottom-0 w-10/12 mb-3">
                                                    ...
                                                    <div className="cardLike flex justify-end">
                                                        <FaHeart size="24" className="mr-1.5" />
                                                        {it.boardHeart}
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div className="Card-ScrollArea-NonResult h-96 flex flex-col col-span-3 justify-center items-center">
                                        ...
                                        <div className="font-['Pre-Bold']">일치하는 결과가 없습니다</div>
                                    </div>
                                )
                            ) : null}
                        </div>
